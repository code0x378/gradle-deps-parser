...
def is_valid_dependency_type(line):
    """Check if the line is a valid gradle dependency type (api, implementation, compile, etc.)"""
...
def get_dependency_type(line):
    """Get the gradle dependency type (api, implementation, compile, etc.) from the line"""
    for dep in DEPENDENCY_TYPES:
        if dep in line:
            return dep
    return None
...
class NormalFormat(ParserFormat):
    def parse(self, text):
        ...
        for line in text.splitlines():
            if is_valid_dependency_type(line) and "group" in line:
                dep = line.split("'")
                deps.append(
                    {
                        "group": dep[1],
...
class KotlinFormat(ParserFormat):
    def parse(self, text):
        deps = []
        for line in text.splitlines():
            if is_valid_dependency_type(line) and ":" in line and "(" in line:
                quoted = re.compile(r'(?:\'|")(.*)(?:\'|")')
                result = quoted.findall(line)
                for c in ["'", '"']:
                    dep = result[0].replace(c, "")
