...
class NormalFormat(ParserFormat):
    ...
    def format_output(self, dep):
        return f"{dep['type']} group: '{dep['group']}', name: '{dep['name']}', version: '{dep['version']}'"
...
class ShortFormat(ParserFormat):
    ...
    def parse(self, text):
        deps = []
        for line in text.splitlines():
            if is_valid_dependency(line) and ":" in line and "name" not in line and "(" not in line:  # skip kolin format
                quoted = re.compile(r'(?:\'|")(.*)(?:\'|")')
...
def main():
    ...
    parser.add_argument('-f', '--output_file', help='Output file', default='data/output.txt')
    parser.add_argument('-i', '--input-format', help='Input format (normal, short, kotlin)', default='normal')
    parser.add_argument('-o', '--output-format', help='Output format (normal, short, kotlin)', default='normal')
